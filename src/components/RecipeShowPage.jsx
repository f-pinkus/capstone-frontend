...
import { ConfirmModal } from "../components/ConfirmModal"; // Adjust this import path if needed
  ...
  return (
    ...
    >
      ...
      {!editing ? (
          ...
          <div className="mb-4">
            ...
            <ul className="mt-2 ps-4">
              {recipe.ingredients.split("\n").filter(Boolean).map((item, index) => (
                <li key={index} className="mb-1">{item.replace(/^(\s*[-*•])\s*/, "")}</li>
              ))}
            </ul>
          </div>
          ...
          <div className="mb-4">
            ...
            <ol className="mt-2 ps-4">
              {recipe.instructions.split("\n").filter(Boolean).map((step, index) => (
                <li key={index} className="mb-2">{step.replace(/^(\s*\d+[\.\)]|\s*[-*•])\s*/, "")}</li>
              ))}
            </ol>
          </div>
        ...
        <div className="card shadow-sm p-4 rounded-4 border-0" style={{ backgroundColor: "#fff" }}>
          {/* Your existing editing form here – unchanged for brevity */}
        </div>
      )}
    </div>
  );
